        ...
        while self.itr < self.n_train_itr:
            ...
            for step in range(self.n_steps):
                ...
                with torch.no_grad():
                    ...
                    output_venv = samples.trajectories
            ...
            if not eval_mode:
                with torch.no_grad():
                    ...
                    logprobs_trajs = torch.cat(
                        [self.model.get_exact_logprobs(obs, s) for obs, s in zip(obs_ts, samples_ts)]
                    )

                    # normalize reward with running variance if specified
                    if self.reward_scale_running:
                        reward_trajs = torch.tensor(
                            self.running_reward_scaler(reward=reward_np.T, first=firsts_np[:-1].T).T,
